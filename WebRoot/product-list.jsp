...
<body>
	<div id="header" class="wrap">
		...
		<div class="help">
			<a href="#" class="shopping">购物车</a><span id="loginOrNot"><a href="login.html">登录</a></span><a
				href="register.html">注册</a><a href="guestbook.html">留言<span id="isAdmin"></span></a>
		</div>
		<div class="navbar">
			<ul class="clearfix">
	...
	<div id="main" class="wrap">
		<div class="lefter">
			...
			<div class="last-view">
				<h2>最近浏览</h2>
				<dl class="clearfix" id="viewHistory">
					<dt>
					...
					<ul class="clearfix">
						<c:choose>
							<c:when test="${totalPage<=0}">
								<li><c:out value="没有可显示的记录"></c:out></li>
							</c:when>
							<c:when test="${totalPage<=5}">
								<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=1">首页</a></li>
								<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=${currentPage-1 }">上一页</a></li>
								<c:forEach var="i" begin="1" end="${totalPage}">
									<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=${i}"> ${i} </a></li>
								</c:forEach>
								<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=${currentPage+1 }">下一页</a></li>
								<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=${totalPage}">末页</a></li>
							</c:when>
							<c:when test="${totalPage>5 and (totalPage-currentPage>5)}">
								<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=1">首页</a></li>
								<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=${currentPage-1 }">上一页</a></li>
								<c:forEach var="i" begin="${currentPage}" end="${currentPage+5}">
									<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=${i}"> ${i} </a></li>
								</c:forEach>
								<li>...</li>
								<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=${currentPage+1 }">下一页</a></li>
							...
							<c:when test="${totalPage>5 and (totalPage-currentPage<=5)}">
								<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=1">首页</a></li>
								<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=${currentPage-1 }">上一页</a></li>
								<c:forEach var="i" begin="${currentPage}" end="${totalPage}">
									<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=${i}"> ${i} </a></li>
								</c:forEach>
								<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=${currentPage+1 }">下一页</a></li>
								<li><a href="ProductServlet?opr=change&classifyId=${classifyId}&page=${totalPage}">末页</a></li>
				...
				<ul class="product clearfix">
					<c:forEach items="${requestScope.goodsList}" var="goods">
						<li>
							<dl>
								<dt>
									<a href="ProductViewServlet?goodsId=${goods.goodsId}" target="_blank">
									<img src="images/product/${goods.path}" /> </a>
								</dt>
								<dd class="title">
									<a href="ProductViewServlet?goodsId=${goods.goodsId}" target="_blank">${goods.goodsName}</a>
								</dd>
